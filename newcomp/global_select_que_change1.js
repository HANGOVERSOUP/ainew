...
export default function Top_select({method,method2,method3,page,routed}) {
  ...
  return (
    ...
    <div>
        ...
        {/* 만드는중 */}        
        <div id ='select_que_raw_outline'>

            <div id='select_que_raw_slect'>
                {datalocation === 'db' ? 
                    <div>
                    {routed != undefined ? 
                    // routed
            ...
            <div id='select_que_raw_select_loaction'>
            ...
            <div id='select_que_raw_model_select_outline1'>

                {/* 조건에 맞춰 버튼을 변경합시다 업로드, 모델실행 등 */}

                {page === 1 ? (
                    <Button id='' onClick={() => method(selectfile)} endIcon={<FileUploadIcon />} focusRipple={true} size="large"  variant="contained" color="primary">
                        데이터 업로드
                     
                    </Button>
                ) : page === 2 ? (
                    <div id='select_que_raw_model_select_outline2'>
                        <FormControl fullWidth sx={{ }}>
                            <InputLabel id="demo-simple-select-label">모델 선택</InputLabel>
                            <Select
                            labelId="demo-simple-select-label"
                            id="demo-simple-select"
                            className='select_que_raw_model_select'
                            // onChange={handleChange}
                            >
                        ...
                        <Button id='select_que_raw_model_btn' onClick={() => method(selectfile)} endIcon={<RestartAltIcon />} focusRipple={true} size="large"  variant="contained" color="primary">
                            모델 실행
                        </Button>
                    </div>
                ) : page === 3 ? (
                    <div id='select_que_model_btn_outline'>
                        <Button id='select_que_model_visual_btn' onClick={() => method()} endIcon={<RestartAltIcon />} size="large" focusRipple={true} variant="contained" color="primary">
                            시각화
                        </Button>
                        <Button id='select_que_model_visual_net' onClick={() => method3()} endIcon={<RestartAltIcon />} size="large" focusRipple={true} variant="contained" color="primary">
                            NET수정
                        </Button>
                    </div>
                ) : page === 4 ? (
        ...
        <div id='select_que_raw_select_ans'>
            {/* 질문선택 */}
            <Autocomplete
                key="unique-key-for-autocomplete"
                disablePortal
